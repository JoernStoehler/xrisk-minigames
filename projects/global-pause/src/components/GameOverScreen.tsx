...
export default function GameOverScreen({ state, onRestart }: Props) {
  ...
  return (
    <div className="h-dvh bg-isia-bg flex items-center justify-center p-6">
      <div className="max-w-sm w-full text-center">
        {won ? (
          <>
            ...
            <p className="text-isia-muted text-sm mb-6 leading-relaxed">
          ...
          <>
            <div className="text-urgency-red text-5xl mb-3">&#10007;</div>
            <h1 className="text-3xl font-bold mb-2 text-urgency-red">
              {state.lossReason
        ...
        {/* Stats */}
        <div className="bg-isia-panel border border-isia-border rounded-lg p-4 mb-6 text-left">
          <h3 className="text-[10px] font-bold uppercase tracking-wider text-isia-muted mb-3">
            Mission Summary
          </h3>
          <div className="grid grid-cols-2 gap-y-1.5 text-xs">
            <div className="text-isia-muted">Duration</div>
            <div className="font-mono text-right">
              {weekToDate(state.week)} ({state.stats.weeksSurvived}w)
            </div>
            <div className="text-isia-muted">Events handled</div>
            <div className="font-mono text-right">{state.stats.eventsHandled}</div>
            <div className="text-isia-muted">Events missed</div>
            <div className="font-mono text-right">{state.stats.eventsIgnored}</div>
            <div className="text-isia-muted">Inspections</div>
            <div className="font-mono text-right">{state.stats.inspectionsLaunched}</div>
            <div className="text-isia-muted">Sanctions</div>
            <div className="font-mono text-right">{state.stats.sanctionsApplied}</div>
        ...
        <button
          onClick={onRestart}
          className="w-full bg-isia-accent hover:bg-isia-accent/80 active:bg-isia-accent/60 text-white font-bold py-3 rounded-lg cursor-pointer transition-colors"
        >
          Try Again
        </button>
      </div>
